const express = require('express');
  ...
  app.post('/upload', verifyToken, upload.fields([
      ...
      try {
          ...
          // Read master_trail_income.xlsx
          const masterFilePath = process.env.NODE_ENV === 'production'
              ? path.join('/app/trail-data', 'master_trail_income.xlsx')
              : path.join(__dirname, 'app', 'trail-data', 'master_trail_income.xlsx');
          let masterData = [];
          try {
              if (fs.existsSync(masterFilePath)) {
                  console.log('Reading master_trail_income.xlsx from:', masterFilePath);
                  const masterWorkbook = XLSX.readFile(masterFilePath);
                  ...
                  console.log('master_trail_income.xlsx does not exist at:', masterFilePath, '. Will create on write.');
              }
              console.log('Master Data before update:', masterData);
          } catch (error) {
              console.error('Error reading master_trail_income.xlsx:', error.message);
              masterData = [];
          }
          ...
          try {
              ...
              console.log('Writing to master_trail_income.xlsx at:', masterFilePath);
              XLSX.writeFile(masterWorkbook, masterFilePath);
              console.log('Updated master_trail_income.xlsx');
          } catch (error) {
              console.error('Error writing master_trail_income.xlsx:', error.message);
